    ...
    def read(self):
        with open(self.filename, 'r') as f:
            ...
            for i in range(int(numEvents)):
                ...
                '''if((i+1>=530836 and i+1<530878) or (i+1>=530879 and i+1<530885)):
                    print f.readline()'''
                '''if(i+1==252677):
                    print f.readline()'''
                cur_timestamp = int(f.read(4).encode('hex'), 16)
                if(j>0):
                    if cur_timestamp>=timestamps[j-1] and cur_timestamp - timestamps[j-1]<=100000000:
                        data[j] = cur_data
                        timestamps[j] = cur_timestamp
                        #print data[j], timestamps[j]
                        j = j+1
