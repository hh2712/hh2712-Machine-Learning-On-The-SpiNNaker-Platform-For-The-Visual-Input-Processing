...
scaler = float(225)/200

def get_data(filename):
    dvs_data = scipy.io.loadmat(filename)
    ts = dvs_data['ts'][0]
    ts = (ts - ts[0])*scaler #from ns to ms
    x = dvs_data['X'][0]
    y = dvs_data['Y'][0]
    p = dvs_data['t'][0]
    return x,y,p,ts
...
def plot_spikes(spikes, title):    
    if spikes is not None:
        ...
        '''for i in range(5):
            ...
            pylab.axvline((i+1)*200,0,1,linewidth = 4, color = 'c',alpha = 0.75,linestyle = 'dashed')'''
        pylab.axvline(2400,0,1,linewidth = 4, color = 'c',alpha = 0.75,linestyle = 'dashed')
        pylab.axvline(4800,0,1,linewidth = 4, color = 'c',alpha = 0.75,linestyle = 'dashed')
        pylab.axvline(7200,0,1,linewidth = 4, color = 'c',alpha = 0.75,linestyle = 'dashed')
...
def plot_prespikes(spikes, title):    
    if spikes is not None:
        pylab.figure()
        ax = plt.subplot(111, xlabel='Time/ms', ylabel='Neruons #', title=title)
        pylab.xlim((0, simtime))
        pylab.ylim((-0.5, prepop_size-0.5))
        lines = pylab.plot([i[1] for i in spikes], [i[0] for i in spikes],".")
        pylab.setp(lines,markersize=10,color='r')
        '''for i in range(5):
            #pylab.axvspan(i*600,(i+1)*600, facecolor='b', alpha=0.5)
...
def data_set(prefix):
    ...
    for j in range(1,num_dataset+1):
        argw.append(get_data(prefix+mid_str+str(j)+postfix))
...
plot_prespikes(pre_spikes, "Spike Pattern of Pre-Synaptic Population")
